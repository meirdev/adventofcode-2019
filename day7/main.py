...
def intcode_computer(opcodes: list[int], inputs: list[str]) -> Iterator[int]:
    opcodes = opcodes[:]
    inputs = inputs[:]
    ...
    while i < len(opcodes):
        ...
        if instruction in [Op.ADD, Op.MUL]:
            ...
            opcodes[opcodes[i + 1]] = inputs.pop(0)
...
def intcode_computer2(opcodes: list[int], inputs: queue.Queue) -> Iterator[int]:
    opcodes = opcodes[:]
    ...
    while i < len(opcodes):
        ...
        if instruction in [Op.ADD, Op.MUL]:
            ...
            opcodes[opcodes[i + 1]] = inputs.get()
...
def run(input: str, in_: list[int]) -> int:
...
def part1(input: str) -> int:
    ...
    for setting in itertools.permutations(range(5)):
        second_input = 0
        for i in setting:
            second_input = run(input, [i, second_input])
